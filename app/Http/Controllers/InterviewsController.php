...
class InterviewsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('front')->with('users',$users)->with('materials', $materials);
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach($users as $user){
            ...
            if($request->input($user->id)==1){
                $probs = DB::select('select id from participants where (start_at>=:start_at or end_at<=:end_at) and :column=1',['start_at'=>$request->input('start_at'),'end_at'=>$request->input('end_at'),'column'=>$column]);
        ...
        return redirect('/interviews')->with('success', 'Material added');
    }
}
